...
> **`classify_model` must support structured outputs.** Every LLM call ships a strict `json_schema`, and a model that lacks that capability does not return an error — the request hangs until the client timeout, and `analyze` fails with a misleading "provider may be slow or unreachable" message. The default (`deepseek-v4-flash`, $0.089/$0.177 per M tokens) is not free for this reason; most free-tier models do not qualify. If you want to swap in a free model, confirm `structured_outputs` appears in its `supported_parameters` at <https://openrouter.ai/api/v1/models> first. `default_model`, which carries the bulk of token volume, is still free by default.
