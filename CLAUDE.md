...
- **JSON structural reliability via strict schemas (0.18.0+).** `LLMClient._complete` requests `response_format={"type": "json_schema", strict: True}` and ships a strictified pydantic schema (via `_strictify_schema` — applies `additionalProperties: false`, expands `required` to all properties, strips `default` keys, recurses into `$defs`). The output shape is enforced server-side by the provider via LiteLLM's translation layer; loose `json_object` mode (pre-0.18.0) led to reproducible malformation on free models with deep schemas.
- **Model choice still matters for quality, not structural reliability.** A stronger model via `--deep-model` for Passes 1 & 3, or `--model openrouter/google/gemini-3.1-flash-lite-preview` for the whole pipeline ($0.25/$1.50 per M tokens), produces deeper reasoning and better terminology. But the JSON-malformation footgun that pre-0.18.0 forced users onto paid models is gone — the free default model now produces structurally valid output for every schema in the codebase.
